...
Переменные для Anki необязательные: пустое значение приводит к использованию стандартных `"Default"` и `"Basic"` соответственно.
...
- **Параметры:**
  ...
  - `items: List[NoteInput | dict]` — список заметок (обязателен). Можно передавать как готовые объекты `NoteInput`, так и плоские словари, где ключи верхнего уровня трактуются как поля заметки и дополнительные атрибуты (`tags`, `images`, `dedup_key`).
...
```json
{
  ...
  "arguments": {
    ...
    "items": [
      ...
      {
        "fields": {"Front": "Столица Германии?", "Back": "Берлин"}
      }
    ]
  }
}
...
- **Параметры:**
  - `noteIds: List[int]` — список идентификаторов заметок (обязателен, минимум один).
- **Ответ:** объект с полем `notes: List[NoteInfo | null]`, где каждая запись содержит структуру заметки или `null`, если соответствующая заметка не найдена.
- **Структура элемента `NoteInfo`:**
  - `noteId: int` — идентификатор заметки.
  - `modelName: str | null` — имя модели.
  - `deckName: str | null` — имя колоды.
  - `tags: List[str]` — список тегов.
  - `fields: Dict[str, str]` — значения полей заметки.
  - `cards: List[int]` — идентификаторы связанных карточек.
- **Пример запроса:**
```json
{
  "name": "anki.note_info",
...
- **Особенности:**
  ...
  - При вставке изображений `target_field` обрабатывается без учёта регистра; если поле не найдено в модели, операция прекращается с ошибкой и заметки не отправляются в Anki — в отличие от `anki.add_from_model`, который лишь добавляет предупреждение `"unknown_target_field"`.
  - Перед вызовом полезно удостовериться, что названия полей совпадают с моделью (например, запросив их через `anki.model_info`).
