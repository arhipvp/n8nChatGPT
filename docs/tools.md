...
- **Поля:**
  - `deck: str` — имя колоды (обязательное).
  - `model: str` — имя модели (обязательное).
...
- **Параметры:**
  - `model: str` — имя модели (обязательный параметр).
...
- **Параметры:**
  - `deck: str` — целевая колода (обязателен).
  - `model: str` — модель карточки (обязателен).
