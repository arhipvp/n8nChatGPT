...
#### Пример вызова `anki.cards_to_notes`
...
## Инструмент `anki.notes_to_cards`

- **Назначение:** возвращает идентификаторы карточек, связанные с указанными заметками, через RPC `notesToCards`.
- **Аргументы:**
  - `noteIds: List[int]` — список идентификаторов заметок (обязательный, минимум один элемент). Строковые значения автоматически приводятся к целым числам.
- **Ответ:** объект `NotesToCardsResponse` с полем `notesToCards`, содержащим отображение `note_id → List[card_id]`. Для заметок без карточек возвращается пустой список.
- **Ошибки:** если AnkiConnect вернул неожиданный формат (например, строку вместо словаря или списка списков), сервер выбросит `ValueError` с пояснением.

#### Пример вызова `anki.notes_to_cards`

```json
{
  "name": "anki.notes_to_cards",
  "arguments": {
    "noteIds": [1655533492601]
  }
}
```
...
{
  "notesToCards": {
    "1655533492601": [1481911573372, 1481911573373]
  }
}
```
