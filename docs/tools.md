...
- **Особенности:**
  ...
  - При вставке изображений `target_field` обрабатывается без учёта регистра; при отсутствии поля в переданных `fields` в ответ добавляется предупреждение `"unknown_target_field"`.
