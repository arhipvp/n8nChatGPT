...
def test_cards_info_normalizes_payload(monkeypatch):
    ...
    assert payload["cardId"] == 101
    assert payload["noteId"] == 55
    assert payload["deckName"] == "Default"
    assert payload["ord"] == 0
    assert payload["interval"] == 10
    assert payload["extra"] == {"did": 999, "flags": 2}


def test_cards_info_rejects_invalid_entry(monkeypatch):
    async def fake_anki_call(action, params):
        assert action == "cardsInfo"
        return [None]
    ...
    with pytest.raises(ValueError):
        asyncio.run(cards_info_fn({"cardIds": [111]}))
